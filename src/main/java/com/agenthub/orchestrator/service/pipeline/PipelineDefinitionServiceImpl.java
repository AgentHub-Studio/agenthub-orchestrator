...
import com.agenthub.orchestrator.exception.PipelineNotFoundException;
import com.agenthub.orchestrator.mapper.PipelineDefinitionMapper;
    ...
    private List<String> findDeadEndNodes(PipelineDefinition pipeline) {
        ...
        for (PipelineNode node : pipeline.nodes()) {
            if (pipeline.getOutgoingEdges(node.id()).isEmpty()) {
                ...
                if (node.type() != com.agenthub.orchestrator.domain.node.NodeType.OUTPUT) {
